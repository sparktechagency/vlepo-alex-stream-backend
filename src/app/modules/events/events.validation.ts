...
export const eventCreateValidationSchema = z.object({
  body: z.object({
    ...
    eventType: z.enum([EVENTS_TYPE.VIRTUAL, EVENTS_TYPE.OFFLINE], {
      required_error: 'Event type is required',
    }),
    ticketPrice: z
      .string({ required_error: 'Ticket price is required' })
      .min(0, 'Ticket price cannot be negative'),
    startTime: z
      .string({
        required_error: 'Start time is required',
        invalid_type_error: 'Start time will be date formate.',
      })
...
export const validateEvent = (data: unknown) =>
  eventCreateValidationSchema.parse(data);
