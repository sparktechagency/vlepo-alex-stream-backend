...
const createEventsIntoDB = async (createdBy: string, payload: IEvent) => {
  ...
  if (!user || user.isDeleted || user.status === USER_STATUS.BLOCKED) {
    throw new ApiError(
      ...
      'User have not permission to create events!'
    );
  }
  ...
  if (ticketPrice <= 6) {
    throw new ApiError(
      ...
      'Minimum ticket price to create event is 6'
    );
  }
...
const getSingleEventByEventId = async (user: JwtPayload, id: string) => {
  ...
  const event = await Event.findById(id)
    .select(
      'createdBy eventName image description eventType ticketPrice startTime soldTicket totalSale ticketSecretCode'
    )
  ...
  const isFavorite = existingUser?.favoriteEvents?.includes(
  ...
  const eventIds = await Payment.find({
    userId: user.id,
  ...
  return {
    ...
    isFavorite,
    isFollowing,
    isTicketBooked: bookedEvents.includes(id),
  };
};

const getSingleSlfEventAnalysisByEventId = async (
  id: string,
  timeframe = '6month'
) => {
  if (!mongoose.isValidObjectId(id)) {
  ...
  const event = await Event.findOne({ _id: id })
    .select(
      'eventName image soldTicket totalSale startTime endTime participants '
    )
  ...
  const events = await Event.aggregate([
    ...
    },
    {
      $group: {
        ...
        doneEvent: {
          $sum: {
            $cond: [{ $eq: ['$status', EVENTS_STATUS.COMPLETED] }, 1, 0],
          },
        },
        upcomingEvent: {
        ...
        cancelEvent: {
          $sum: {
            $cond: [{ $eq: ['$status', EVENTS_STATUS.CANCELLED] }, 1, 0],
          },
        },
        liveEvent: {
...
const getAllEvents = async (
  ...
  const favoriteEvents = isExistUser?.favoriteEvents?.map(event =>
    event.toString()
...
const getMyFavouriteEvents = async (
  query: Record<string, unknown>,
  userId: string
) => {
  ...
  const events = new QueryBuilder(
    Event.find({ categoryId: { $in: me?.selectedCategory } }),
    query
  )
    .paginate()
    .sort()
...
const getAllEventsOfCreator = async (
  query: Record<string, unknown>,
  creatorId: string
) => {
...
const cancelMyEventById = async (eventId: string, creatorId: string) => {
  ...
  if (creatorId !== existEvent?.createdBy.toString()) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
      'You can not cancel event of another user.'
    );
  }
...
const updateAllEventsTrendingStatus = async () => {
  const events = await Event.find().select(
    'isTrending soldSeat views startTime status'
  );
    ...
    if (
      ...
      if ((event.views as number) > 1000) {
        ...
        const timeDifference =
          (event.startTime.getTime() - currentTime.getTime()) /
          (1000 * 60 * 60); // difference between hour
...
const getFollowingUserEvents = async (
  ...
  const result = await Event.find(whereConditions).populate({
    path: 'createdBy',
    select: { name: 1, photo: 1 },
  });

  return {
    stat: { upcomingEvent: upcomingEvents, completedEvents: completedEvents },
    result: result,
  };
};
